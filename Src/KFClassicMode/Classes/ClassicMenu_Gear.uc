class ClassicMenu_Gear extends KFGFxObject_Menu;

var ClassicPlayerReplicationInfo ClassicPRI;
...
function InitializeMenu( KFGFxMoviePlayer_Manager InManager )
{
    ...
    LocalizeText();
    EmoteList = class'ClassicEmoteList'.static.GetEmoteArray();
    InitCharacterMenu();
    TraderItems = KFGameReplicationInfo( GetPC().WorldInfo.GRI ).TraderItems;
}
function InitCharacterMenu()
{
    ClassicPRI = ClassicPlayerReplicationInfo(GetPC().PlayerReplicationInfo);
    
    if( ClassicPRI!=None && ClassicPRI.bClientInitChars )
        CharListRecieved();
    else if( ClassicPRI==None )
    {
    ...
    {
        ClassicPRI.OnCharListDone = CharListRecieved;
        bWaitingCharList = true;
    }
}
...
function UpdateEmoteList()
{
    ...
    {
        if ( class'ClassicEmoteList'.static.GetUnlockedEmote(EmoteList[i].Id, ClassicPlayerController(GetPC())) != 'NONE')
        {
...
function UpdateCharacterList()
{
    ...
    ItemIndex = 0;
    DataProvider = CreateArray();
    for( i=0; i<ClassicPRI.CharacterArchetypes.length; i++)
    {
        SlotObject = CreateObject( "Object" );
        SlotObject.SetInt("ItemIndex", i);
        SlotObject.SetString("label", Localize(String(ClassicPRI.CharacterArchetypes[i].Name), "CharacterName", class'KFGFxMenu_Gear'.Default.KFCharacterInfoString));
        SlotObject.SetBool("enabled", true);
        TexturePath = "img://"$PathName(ClassicPRI.CharacterArchetypes[i].DefaultHeadPortrait);
        SlotObject.SetString("source", TexturePath);
        DataProvider.SetElementObject(ItemIndex, SlotObject);
        ItemIndex++;
    }
    for( i=0; i<ClassicPRI.CustomCharList.length; i++)
    {
        if( !ClassicPRI.IsClientCharLocked(ClassicPRI.CharacterArchetypes.length+i) )
        {
            SlotObject = CreateObject( "Object" );
            SlotObject.SetInt("ItemIndex", (ClassicPRI.CharacterArchetypes.length+i));
            SlotObject.SetString("label", Repl(string(ClassicPRI.CustomCharList[i].Char.Name),"_"," "));
            SlotObject.SetBool("enabled", true);
            TexturePath = "img://"$PathName(ClassicPRI.CustomCharList[i].Char.DefaultHeadPortrait);
            SlotObject.SetString("source", TexturePath);
            DataProvider.SetElementObject(ItemIndex, SlotObject);
            ItemIndex++;
        }
...
function UpdateGear()
{
    ...
    CurrentCharInfo = ClassicPRI.GetSelectedArch();
    bIsCustomChar = ClassicPRI.ReallyUsingCustomChar();
...
function SetCurrentCharacterButtons()
{
    ...
    bCustom = ClassicPRI.UsesCustomChar();
    DataObject = CreateObject("Object");
    
    CharacterIndex = ClassicPRI.RepCustomizationInfo.CharacterIndex;
    HeadMeshIndex = ClassicPRI.RepCustomizationInfo.HeadMeshIndex;
    HeadSkinIndex = ClassicPRI.RepCustomizationInfo.HeadSkinIndex;
    BodyMeshIndex = ClassicPRI.RepCustomizationInfo.BodyMeshIndex;
    BodySkinIndex = ClassicPRI.RepCustomizationInfo.BodySkinIndex;
    if( bCustom )
    {
        CharacterIndex = ClassicPRI.CustomCharacter.CharacterIndex;
        HeadMeshIndex = ClassicPRI.CustomCharacter.HeadMeshIndex;
        HeadSkinIndex = ClassicPRI.CustomCharacter.HeadSkinIndex;
        BodyMeshIndex = ClassicPRI.CustomCharacter.BodyMeshIndex;
        BodySkinIndex = ClassicPRI.CustomCharacter.BodySkinIndex;
    }
...
function SetEmoteButton()
{
    ...
    EmoteIndex = class'ClassicEmoteList'.static.GetEmoteIndex( class'ClassicEmoteList'.static.GetEquippedEmoteId(ClassicPlayerController(GetPC())));
...
function SetAttachmentButtons(string AttachmentMeshKey, string sectionFunctionName)
{
    ...
    {
        AttachmentIndex = bCustom ? ClassicPRI.CustomCharacter.AttachmentMeshIndices[i] : ClassicPRI.RepCustomizationInfo.AttachmentMeshIndices[i];        
...
function Callback_Emote(int Index)
{
    ...
    if( KFPC != none )
    {
        class'ClassicEmoteList'.static.SaveEquippedEmote(EmoteList[Index].ID, ClassicPlayerController(KFPC));

        if ( ClassicPawn_Customization(KFPC.Pawn) != none )
        {
            ClassicPawn_Customization(KFPC.Pawn).PlayEmoteAnimation();
        }
    }
...
{
    ClassicPRI.ChangeCharacter(Index,!ClassicPRI.UsesCustomChar());
    UpdateGear();
}
...
function Callback_Head( int MeshIndex, int SkinIndex )
{
    ...
    ClassicPRI.UpdateCustomization(CO_Head, MeshIndex, SkinIndex);
...
function Callback_Body( int MeshIndex, int SkinIndex )
{
    ...
    ClassicPRI.UpdateCustomization(CO_Body, MeshIndex, SkinIndex);
...
function Callback_Attachment( int MeshIndex, int SkinIndex )
{
    ...
    {
        if( MeshIndex==`CLEARED_ATTACHMENT_INDEX )
            ClassicPRI.RemoveAttachments();
        else
        {
            class'ClassicCharacterInfo'.Static.DetachConflictingAttachments(CurrentCharInfo, MeshIndex, KFP, ClassicPRI);
            SlotIndex =  class'ClassicCharacterInfo'.Static.GetAttachmentSlotIndex(CurrentCharInfo, MeshIndex, KFP, ClassicPRI);
...
defaultproperties
{
    ...
    SubWidgetBindings.Add((WidgetName="perkSelectionContainer",WidgetClass=class'ClassicGearContainer_PerksSelection'))
}