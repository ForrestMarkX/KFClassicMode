...
var KFGUI_TextScroll NewsField;
...
function InitMenu()
{
	...
	// Client settings
	NewsField = KFGUI_TextScroll(FindComponentID('News'));
...
function Timer()
{
	if( !ClassicPlayerController(GetPlayer()).bMOTDReceived )
		SetTimer(0.2,false);
	else NewsField.SetText(ClassicPlayerController(GetPlayer()).ServerMOTD);
}
...
defaultproperties
{
	Begin Object Class=KFGUI_TextScroll Name=NewsText
		ID="News"
		XPosition=0.025
		YPosition=0.025
		XSize=0.95
