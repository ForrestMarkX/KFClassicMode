class ClassicEmoteList extends Object
    abstract;

var const array<Emote> Emotes;

static final function name GetUnlockedEmote( int ItemId, ClassicPlayerController PC )
{
    local int i;
    ...
    if( i != INDEX_NONE )
    {
        return default.Emotes[i].AnimName;
    }

    return 'NONE';
}

static final function SaveEquippedEmote( int ItemId, ClassicPlayerController PC )
{
    if( PC == None )
        return;
...
{
    return default.Emotes.Find('Id', ItemId);
}

static final function int GetEquippedEmoteId(ClassicPlayerController PC)
{
    if( PC == None )
        return -1;
