...
function DrawDailyEntry( Canvas C, int Index, float YOffset, float Height, float Width, bool bFocus )
{
    ...
    local string Title, Description, Progress, RewardT;
    ...
    RewardT = class'KFMission_LocalizedStrings'.default.RewardString;
    ...
    if( bIsCompleted )
    {
        ...
        C.SetPos(TextX, TextY);
        C.DrawText(Localize("BanImporter", "msgStatusDone", "WebAdmin"),, FontScalar, FontScalar);
    }
    ...
    C.TextSize(RewardT, XS, YS, FontScalar, FontScalar);
    C.SetDrawColor(255, 255, 255, 255);
    C.SetPos(RewardX-XS-(Owner.HUDOwner.ScaledBorderSize*2), RewardY + (RewardH/2) - (YS/2));
    C.DrawText(RewardT,, FontScalar, FontScalar);
}
