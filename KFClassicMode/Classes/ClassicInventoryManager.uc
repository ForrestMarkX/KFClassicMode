...
simulated function int GetAdjustedSellPriceFor( const out STraderItem OwnedItem, optional KFWeapon OwnedWeapon )
{
	...
	OriginalSellPrice = Super.GetAdjustedSellPriceFor(OwnedItem, OwnedWeapon);
	KFPC = KFPlayerController(Instigator.Controller);
	...
	if( KFPC != None )
	{
		...
		if( Index != INDEX_NONE )
		{
			OriginalSellPrice *= SavedPrices[Index].Perk.GetCostScaling(SavedPrices[Index].PerkLevel, OwnedItem, OwnedWeapon);
		}
	}
