class KFHUDInterface extends KFGFxHudWrapper_Versus
    config(ClassicHUD);
...
function PostRender()
{
    ...
    local ClassicMoviePlayer_Manager HUDManager;
    local KFGFxMenu_Perks PMenu;
    ...
    HUDManager = ClassicMoviePlayer_Manager(KFPlayerOwner.MyGFxManager);
    if( HUDManager != None )
    {    
        PMenu = HUDManager.PerksMenu;
        if( PMenu != None )
        {
            if( PMenu.SkillsContainer != None )
            {
                DI = PMenu.SkillsContainer.GetDisplayInfo();
                if( DI.Visible )
                {
                    DI.Visible = false;
                    PMenu.SkillsContainer.SetDisplayInfo(DI);
                }
            }
            
            if( PMenu.SkillsSummaryContainer != None )
            {
                DI = PMenu.SkillsSummaryContainer.GetDisplayInfo();
                if( DI.Visible )
                {
                    DI.Visible = false;
                    PMenu.SkillsSummaryContainer.SetDisplayInfo(DI);
                }
            }                    
        }
        
        HUDManager.HideFlashButtons();
    }
...
function DrawHUDBox
    ...
    float Height, 
    coerce string Text, 
    EJustificationType Justification,
    optional int JustificationPadding,
    optional Color TextColor,
    optional Texture IconTex, 
    optional float Alpha=-1.f,
    optional float IconScale=1.f, 
    optional float TextScale=1.f, 
    optional array<String> StringArray
    )
{
    ...
    if( !bLightHUD )
    {
        GUIStyle.DrawOutlinedBox(X, Y, Width, Height, ScaledBorderSize, MakeColor(HudMainColor.R, HudMainColor.G, HudMainColor.B, bUseAlpha ? byte(FMin(Alpha, HudMainColor.A)) : HudMainColor.A), MakeColor(HudOutlineColor.R, HudOutlineColor.G, HudOutlineColor.B, bUseAlpha ? byte(Min(Alpha, HudOutlineColor.A)) : HudOutlineColor.A));
    }
    ...
    if( IconTex != None )
    {
        IconXL = TempX + (IconScale / 2);
        IconYL = Y + (Height / 2) - (IconScale / 2);
        
        if( HudOutlineColor != DefaultHudOutlineColor )
            Canvas.SetDrawColor(HudOutlineColor.R, HudOutlineColor.G, HudOutlineColor.B, bUseAlpha ? byte(Alpha) : 255);
        else Canvas.SetDrawColor(255, 255, 255, bUseAlpha ? byte(Alpha) : 255);
        
        Canvas.SetPos(IconXL, IconYL);
        
        if( IconScale == 1.f )
        {
            IconScale = IconTex.GetSurfaceHeight();
        }
        
        Canvas.DrawRect(IconScale - ScaledBorderSize, IconScale - ScaledBorderSize, IconTex);
    }

    Canvas.SetDrawColor(TextColor.R, TextColor.G, TextColor.B, bUseAlpha ? byte(Min(Alpha, TextColor.A)) : TextColor.A);
    ...
    {
        case HUDA_Right:
            X += Width + GUIStyle.ScreenScale(JustificationPadding);
            break;
        case HUDA_Left:
            X -= Width + GUIStyle.ScreenScale(JustificationPadding);
            break;
        case HUDA_Top:
            Y += Height + GUIStyle.ScreenScale(JustificationPadding);
            break;
        case HUDA_Bottom:
            Y -= Height + GUIStyle.ScreenScale(JustificationPadding);
            break;
...
function RenderKFHUD(KFPawn_Human KFPH)
{
    ...
    if( !bHidePlayerInfo )
    {
        ...
        DrawHUDBox(BoxXL, BoxYL, BoxSW, BoxSH, string(KFPH.Health), HUDA_Right,, HealthFontColor, HealthIcon,, scale_w2, FontScalar);
    
        // Armor
        DrawHUDBox(BoxXL, BoxYL, BoxSW, BoxSH, string(KFPH.Armor), HUDA_Right,, FontColor, ArmorIcon,, scale_w2, FontScalar);
        
        if( Inv != None )
        {
            // Weight
            BoxSW = SizeX * 0.082;
            DrawHUDBox(BoxXL, BoxYL, BoxSW, BoxSH, Inv.CurrentCarryBlocks$"/"$Inv.MaxCarryBlocks, HUDA_Right,, FontColor, WeightIcon,, scale_w2, FontScalar);
        }
    ...
    if( CurrentWeapon != None )
    {
        if( !bHideWeaponInfo )
        {
            ...
            {
                // Grenades
                DrawHUDBox(BoxXL, BoxYL, BoxSW, BoxSH, string(Inv.GrenadeCount), HUDA_Left,, FontColor, GrenadesIcon,, scale_w2, FontScalar);
            }
            ...
            if( CurrentWeapon.UsesAmmo() || (CurrentWeapon.IsA('KFWeap_Welder') || CurrentWeapon.IsA('KFWeap_Healer_Syringe')) )
            {
                ...
                // Clips
                DrawHUDBox(BoxXL, BoxYL, BoxSW, BoxSH, string(MagCount), HUDA_Left,, FontColor, GetClipIcon(CurrentWeapon, bSingleFire),, scale_w2, FontScalar);
                
                // Bullets
                if( !bSingleFire )
                {
                    DrawHUDBox(BoxXL, BoxYL, BoxSW, BoxSH, string(AmmoCount), HUDA_Left,, FontColor, GetBulletIcon(CurrentWeapon),, scale_w2, FontScalar);
                }
                
                // Secondary Ammo
                if( bHasSecondaryAmmo )
                {
                    DrawHUDBox(BoxXL, BoxYL, BoxSW, BoxSH, string(CurrentWeapon.GetSecondaryAmmoForHUD()), HUDA_Left,, FontColor, GetSecondaryAmmoIcon(CurrentWeapon),, scale_w2, FontScalar);
                }
            }
            ...
            {
                DrawHUDBox(BoxXL, BoxYL, BoxSW, BoxSH, string(int(KFPH.BatteryCharge)), HUDA_Left,, FontColor, KFPH.bFlashlightOn ? FlashlightIcon : FlashlightOffIcon,, scale_w2, FontScalar);
            }
            ...
            if ( bDisplayQuickSyringe && !CurrentWeapon.IsA('KFWeap_Healer_Syringe') )
            {
                ...
                if( S != None )
                {
                    ...
                    if ( TempSize < QuickSyringeDisplayTime )
                    {
                        ...
                        DrawHUDBox(BoxXL, BoxYL, BoxSW, BoxSH, string(S.AmmoCount[0]), HUDA_Left,, FontColor, SyringIcon, FadeAlpha, scale_w2, FontScalar);
                    }
                    else
                    {
...
function DrawInventory()
{
    ...
    for ( i = 0; i < MAX_WEAPON_GROUPS; i++ )
    {
        ...
        DrawHUDBox(TempX, TempY, TempWidth, TempHeight * 0.25, GetWeaponCatagoryName(i), HUDA_Top, 24, FontColor,, FadeAlpha,, CatagoryFontScalar);
...
function DrawVOIPStatus()
{
    ...
    for( i=0; i <= 4 && i < TalkerPRIs.Length; i++ )
    {
        ...
        DrawHUDBox(X, Y, W, H, PlayerName, HUDA_Top, 12, WhiteColor, VoiceChatIcon,, H, TextScale);
    }
}
...
defaultproperties
{
    ...
    HumanHUDClass=class'ClassicMoviePlayer_HUD'
